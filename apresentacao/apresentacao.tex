...
\frame{
	\begin{block}{O que é Otimização Multi-objetivo?}
		  \begin{itemize}
		   \item Se refere a otimização simultânea de mais de um 
objetivo.
		  \end{itemize}
	\end{block}
	\begin{block}{O que é uma solução em um problema Multi-objetivo?}
	  Na otimização multi-objetivo a solução pode não ser única, 
mas um conjunto de soluções incomparáveis entre si. Cada solução é representada 
por um conjunto de valores no espaço de decisão e o conjunto de valores dos 
objetivos no espaço de objetivos.\\
Esse conjunto pode ser 
encontrado por meio da dominância de Pareto. O conjunto de soluções não 
dominadas, é então, o conjunto de soluções para o problema.\\
	\begin{itemize}
	 \item 	Uma solução é dita ótima (não dominada) se não existe nenhuma 
outra 
solução melhor do que ela em algum objetivo, sem ser pior em outro.
	 \item Uma solução é dita dominar outra, se for melhor em pelo menos um 
objetivo sem ser pior nos demais.
	\end{itemize}
	\end{block}
}
...
0.3]{figuras/exemplo.png}
			\caption{Exemplo de problema multi-objetivo}
			\label{fig:exemplo}
		\end{figure}
	\end{minipage}
}
...
\frame{
	...
	\begin{block}{Distribuição Gaussiana}
	  \begin{equation}
	   f(x, \mu, \sigma) = \frac{1}{\sqrt{2 \pi \sigma^2}} e^{\left ( 
